...
O presente trabalho buscou consolidar e tratar as bases de dados. Posteriormente, se utilizou a base consolidada para a
construção de um Dashboard no Power BI.
...
-- Unir as duas bases da Perfect Pay (PP) e selecionar as colunas que vão ser utilizadas
...
set ValordaVenda = isnull(ValordaVenda, [Preço do produto])
	...
	-- Preencher valores vazios da coluna de Valor da Comissão Convertido (usando valores da coluna de comissão gerada)
...
set País = 'Exterior'
	...
	-- Criar coluna de produto ('xxxx')
	-- Os nomes dos produtos foram ocultados propositalmente
...
case
when Produto = 'aaaa' then 'bbbb'
when Produto = 'cccc' then 'dddd'
when Produto = 'eeee' then 'ffff'
when Produto = 'gggg' then 'hhhh'
when Produto = 'iiii' then 'jjjj'
when Produto = 'kkkk' then 'llll'
else Produto
...
-- Agrupar status que significam a mesma coisa na coluna de status
...
-- Agrupar formas de pagamento com menos de 20 transações em 'outros'
